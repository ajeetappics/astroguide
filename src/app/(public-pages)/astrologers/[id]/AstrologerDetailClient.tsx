...
export interface AstrologerDetailClientProps {
  astroId: string;
  initialAstro?: any;
  initialFeedbacks?: any;
}

export default function AstrologerDetailClient({
  astroId,
  initialAstro,
  initialFeedbacks
  ...
  const [isLoading, setIsLoading] = useState(!initialAstro);
  ...
  useEffect(() => {
    ...
    // If initial data is already provided from server, skip re-fetching on mount
    if (initialAstro && initialFeedbacks) {
      setIsLoading(false);
      return;
    }
    ...
    const loadDetails = async () => {
      try {
        if (!initialAstro) setIsLoading(true);
        const data = initialAstro || (await fetchAstrologerById(astroId));
        if (isMounted && data) {
          setAstro(data);
          const feedbackTargetId = data._id || data.id || astroId;
          const feedbacks = initialFeedbacks || (await fetchAstrologerFeedbacks(feedbackTargetId, 1, REVIEWS_PER_PAGE));
  ...
  }, [astroId, initialAstro, initialFeedbacks]);
  ...
  // Combine all photos, certificates, and certificate gallery into a single unified Photo Gallery
  ...
  const consultationPrice =
    ...
    currentAstro.chat?.ratePerMinute ||
  ...
  const faqList = [
    ...
    },
    {
      ...
      answer: `${astroName} offers astrology consultations in ${languagesList.join(', ')}, ensuring you can discuss your concerns comfortably in your preferred language.`
    },
    {
    ...
    },
    {
      ...
      answer: `For an accurate Kundali and astrological reading, please provide your exact Date of Birth, Time of Birth, and Place of Birth. If birth time is unknown, ${astroName} can also guide you through Prashna Kundali (Horary Astrology) or Tarot reading.`
    },
    {
    ...
    <div className="min-h-screen bg-[#FFFDF9] pb-20 font-helvetica">
      ...
      <div className="relative w-full bg-[#fdf7e1] overflow-hidden pt-[70px] lg:pt-[80px] pb-10 md:pb-12">
        ...
        <div className="container mx-auto max-w-6xl px-4 relative z-10 py-4 md:py-5">
          {/* Breadcrumbs */}
          <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs md:text-sm font-bold tracking-wider uppercase text-[#4A2B23]/70">
            ...
            <BsChevronRight className="text-[9px] shrink-0" />
            <span className="text-[#F6971E] truncate max-w-[200px] sm:max-w-none">{astroName}</span>
          </nav>
        </div>
      </div>

      {/* Main Container (Pulled up to overlap banner with equal spacing) */}
      <main className="container mx-auto max-w-6xl px-4 relative -mt-10 md:-mt-12 z-20">
            ...
            <div className="flex-1 min-w-0 w-full pt-1">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 w-full">
                ...
                <div className="w-full md:w-auto flex flex-col items-center md:items-end gap-2.5 mt-2 md:mt-0 shrink-0">
                  ...
                  <Link
                    href={`${process.env.NEXT_PUBLIC_URL}/astrologer-profile?astroId=${currentAstro?._id}`}
                    onClick={() => {
                      try {
        ...
        <div className="w-full space-y-8">
          ...
          <section className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-7 shadow-sm border border-[#F6971E]/15">
            ...
            <div className="prose max-w-none text-gray-600 text-xs sm:text-sm md:text-[15px] leading-relaxed space-y-3 font-normal">
              {currentAstro.profileBio || currentAstro.bio || currentAstro.about ? (
                ...
                <p>
                  {astroName} is a verified astrologer with {currentAstro.experience || 5} years of experience in Vedic astrology, offering insightful guidance for career, relationships, marriage, and spiritual growth.
                </p>
              )}
            </div>

            {/* Key Highlights / AEO Summary Table */}
            <div className="mt-6 pt-5 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              <div className="p-2.5 rounded-xl bg-[#FFFDF9] border border-[#F6971E]/15">
                <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider block">Speciality</span>
                <span className="text-xs sm:text-sm font-bold text-[#4A2B23] mt-0.5 block truncate">
                  {expertiseList[0] || 'Vedic Astrology'}
                </span>
              </div>
              <div className="p-2.5 rounded-xl bg-[#FFFDF9] border border-[#F6971E]/15">
                <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider block">Experience</span>
                <span className="text-xs sm:text-sm font-bold text-[#4A2B23] mt-0.5 block">
                  {currentAstro.experience || 5}+ Years
                </span>
              </div>
              <div className="p-2.5 rounded-xl bg-[#FFFDF9] border border-[#F6971E]/15">
              ...
              <div className="custom-x-scroll flex gap-3.5 sm:gap-4 overflow-x-auto pb-4 pt-1 scroll-smooth">
                {allGalleryPhotos.map((photoUrl: any, idx: any) => (
                  <div
                    ...
                    className="w-[160px] sm:w-[190px] md:w-[210px] lg:w-[calc((100%-64px)/5)] lg:min-w-[calc((100%-64px)/5)] flex-shrink-0 group relative aspect-square rounded-2xl overflow-hidden bg-gray-100 border border-gray-200 hover:border-[#F6971E] transition-all cursor-pointer shadow-2xs hover:shadow-md"
                  >
                    <img
                      src={photoUrl}
                      alt={`${astroName} certificate or gallery photo ${idx + 1}`}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                      loading="lazy"
                      onError={(e) => {
                        ...
                        <div className="flex items-start gap-2.5 sm:gap-3 mb-2 sm:mb-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="font-bold text-[#4A2B23] text-xs sm:text-sm md:text-[15px] truncate">
                                {review.name}
                              </span>
                ...
                {totalReviewPages > 1 && (
                  <div className="mt-6 pt-5 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-xs sm:text-sm text-gray-500 font-helvetica order-2 sm:order-1">
                      Showing <span className="font-bold text-[#4A2B23]">{(reviewPage - 1) * REVIEWS_PER_PAGE + 1}</span> -{' '}
                      <span className="font-bold text-[#4A2B23]">
                        {Math.min(reviewPage * REVIEWS_PER_PAGE, totalReviewsCount)}
                      </span>{' '}
                      of <span className="font-bold text-[#4A2B23]">{totalReviewsCount}</span> reviews
                    </p>
          ...
          {/* 5. AEO & SEO: Visible Frequently Asked Questions (FAQ) Section */}
          <section
            id="faq-section"
            className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-sm border border-[#F6971E]/15"
            aria-labelledby="faq-heading"
          >
            <div className="mb-4 sm:mb-6 border-b border-gray-100 pb-3">
              <h2
                id="faq-heading"
                className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold font-['Inria_Serif'] text-[#4A2B23] flex items-center gap-2 sm:gap-2.5"
              >
                <span className="w-1 sm:w-1.5 h-4 sm:h-5 md:h-6 bg-[#F6971E] rounded-full inline-block shrink-0"></span>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className="w-full p-3.5 sm:p-4 text-left flex items-center justify-between gap-3 font-semibold text-xs sm:text-sm md:text-[15px] text-[#4A2B23] cursor-pointer"
                      aria-expanded={isOpen}
                    >
                      <span className="flex items-center gap-2.5 min-w-0">
                        <BsQuestionCircleFill className="text-[#F6971E] text-sm shrink-0" />
                    ...
                    {isOpen && (
                      <div className="px-3.5 sm:px-4 pb-4 pt-1 text-xs sm:text-sm text-gray-600 leading-relaxed border-t border-[#F6971E]/10 mt-1">
                        <p>{faq.answer}</p>
                      </div>
                    )}
                  </div>

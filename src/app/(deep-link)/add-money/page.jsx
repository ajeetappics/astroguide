...
export default function AddMoneyRedirect() {
    const returnToSource = () => {
        ...
        try {
            ...
            // Clear session flags
            sessionStorage.removeItem("redirect_in_progress_add_money");
            sessionStorage.removeItem("deep_link_source");

            if (savedSource && !savedSource.includes("/add-money")) {
                window.location.replace(savedSource);
                return;
            }

            if (referrer && referrer.startsWith(origin) && !referrer.includes("/add-money")) {
    ...
    useEffect(() => {
        ...
        if (isBackNav() || sessionStorage.getItem("redirect_in_progress_add_money")) {
            returnToSource();
            return;
        }

        // Mark that redirect has been initiated in this tab session
        sessionStorage.setItem("redirect_in_progress_add_money", "true");

        // 3. Handle page restore from Back-Forward Cache (bfcache)
        ...
        <div className="min-h-screen bg-gradient-to-b from-[#FFFDF9] via-[#FFF8EE] to-[#FFF3E0] flex flex-col items-center justify-center p-4 sm:p-6 font-helvetica relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-white/95 backdrop-blur-md rounded-3xl shadow-[0_12px_45px_rgba(246,151,30,0.14)] border border-[#F6971E]/25 p-6 sm:p-8 text-center relative z-10">
                ...
                <p className="text-gray-600 text-xs sm:text-sm leading-relaxed mb-6">
                    Taking you directly to the wallet page in the app.
                </p>

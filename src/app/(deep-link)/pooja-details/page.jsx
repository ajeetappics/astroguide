...
export default function PoojaDetailsRedirect() {
    ...
    const returnToSource = () => {
        ...
        try {
            ...
            sessionStorage.removeItem("redirect_in_progress_pooja");
            sessionStorage.removeItem("deep_link_source");

            if (savedSource && !savedSource.includes("/pooja-details")) {
                window.location.replace(savedSource);
                return;
            }

            if (referrer && referrer.startsWith(origin) && !referrer.includes("/pooja-details")) {
    ...
    useEffect(() => {
        ...
        if (isBackNav() || sessionStorage.getItem("redirect_in_progress_pooja")) {
            returnToSource();
            return;
        }

        // Mark that redirect has been initiated in this tab session
        sessionStorage.setItem("redirect_in_progress_pooja", "true");
        ...
        const path = `/pooja-details?poojaId=${poojaId || ""}`;
